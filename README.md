# 药品研发质量体系知识库 · 9527 法规问答（一键部署仓库）

把「药品研发质量知识库前端」+「9527 法规问答实时后端」打包成一个可一键部署的 Git 仓库。
**Git 主仓使用 Gitee**；后端 FastAPI 同源托管前端静态站 + `/api/qa` 检索接口，调用本地 `kb.sqlite`（约 3096 篇法规）。

> **与 WorkBuddy 9527 技能一致**：网页回答严格采用 9527 技能的四段式
> 【结论】→【法规依据】→【适用提示】→【时效说明】，引用格式为
> 《标题》（发布机构，文号，发布日期，状态）> 原文摘录 本地：路径 来源：URL，
> 并按效力层级排序、自动做时效核验。数据来自同一份 `kb.sqlite`，因此与 WorkBuddy 内 9527 的回复一致。
...
```bash
cd pharma-kb-render
git init && git add -A && git commit -m "pharma-kb-render: 9527 实时问答 + 对话式前端"
...
> Gitee 主仓 + Docker 自托管 = 既可 `git clone` 直接拉最新代码，又能稳定对外提供 9527 实时问答。
