 ...
 *   · 生产工序（有序步骤）/ 工艺特点
 *   · 质控重点（生产过程控制要点，摘要式）
 *   · 质量控制要点（检验项目 / 方法 / 标准·限度，表格式，QC 实验室视角）
 *   · 主要依据·指导原则
 *
...
(function () {
  ...
  // 条目（生产工序 / 工艺特点 / 质控重点 / 质量控制要点 / 主要依据）
  // qc_points 结构：{ t:'检验项目', m:'方法/检测手段', s:'标准·可接受限度' }
  const ENTRIES = [
    /* ---------------------- 化学药 · 原料药 ---------------------- */
    {
  ...
  function detailHtml(e) {
    ...
    const qcPoints = (e.qc_points || []).map(function (p) {
      return '<tr><td class="mf-qcp-t">' + esc(p.t) + '</td>' +
             '<td class="mf-qcp-m">' + esc(p.m) + '</td>' +
             '<td class="mf-qcp-s">' + esc(p.s) + '</td></tr>';
    }).join('');
  ...
  function paintBody() {
    ...
    body.innerHTML = mfState.entryId
      ? detailHtml(ENTRIES.find(function (e) { return e.id === mfState.entryId; }) || { steps: [], features: [], qc: [], qc_points: [], regs: [] })
      : listHtml(list);
